...
//    var list:Array<SongMetadata> = []
    var list = [
        SongMetadata(id: 3, name: "Mary Had a Little Lamb", highScore: 1000, rank: "S", level: 1),
        SongMetadata(id: 4, name: "Joy to the World", highScore: 2000, rank: "S", level: 1),
        SongMetadata(id: 5, name: "Minuet in G", highScore: 1500, rank: "S", level: 1),
    ]
...
//            let artist = (json["artist"] as AnyObject? as? String) ?? ""
            ...
            list.append(SongMetadata(id: id, name: name, highScore: scoresDict[id] ?? 0, rank: "A", level: level))
        }
    }
...
struct SelectMusic: View {
    ...
    @State private var songMetadata: SongMetadata = SongMetadata(id: -1, name: "", highScore: -1, rank: "", level: -1)
    // List of all songs
...
struct SelectMusic_Previews: PreviewProvider {
    static var previews: some View {
        SelectMusic(allSongs: [SongMetadata(id: -1, name: "", highScore: -1, rank: "", level: -1)], isNavigationBarHidden: .constant(false)).previewLayout(.fixed(width: 896, height: 414))
    }
}
